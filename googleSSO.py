...
from app_config import get_public_base_url, parse_email_set
...
async def google_callback(request: Request):
    ...
    try:
        # redirect_uri is already in the OAuth session from authorize_redirect;
        # passing it again duplicates the kwarg inside fetch_access_token.
        ...
        admin_emails = parse_email_set("ADMIN_EMAILS")
        ir_emails = parse_email_set("IR_EMAILS")
        ...
        if user_role == "ir":
            return RedirectResponse(url="/admin/ir/links")
        return RedirectResponse(url="/login?error=admin_only")
...
def add_google_login_button():
    ...
    with ui.link(target="/auth/google/login", new_tab=False).classes("w-full").style(
        "text-decoration: none;"
    ):
        with ui.row().classes(
            "w-full bg-white border border-gray-300 rounded px-4 py-2 items-center justify-center gap-3 hover:bg-gray-50 cursor-pointer"
        ):
            ui.html(
                """
